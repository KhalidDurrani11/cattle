            ...
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="bg-gray-800/50 p-6 rounded-2xl shadow-lg border border-gray-700/50 mb-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <input type="text" placeholder="Search by breed, location..." name="searchTerm" value={filters.searchTerm} onChange={handleFilterChange} className="lg:col-span-4 bg-gray-900/50 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-teal-500" />
                        
                        <select name="breed" value={filters.breed} onChange={handleFilterChange} className="bg-gray-900/50 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-teal-500">
                            <option>Any Breed</option>
                            {uniqueBreeds.map(b => <option key={b}>{b}</option>)}
                        </select>
                         <select name="location" value={filters.location} onChange={handleFilterChange} className="bg-gray-900/50 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-teal-500">
                            <option>Any Location</option>
                            {uniqueLocations.map(l => <option key={l}>{l}</option>)}
                        </select>
                        <select name="isVerified" value={filters.isVerified} onChange={handleFilterChange} className="bg-gray-900/50 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-teal-500">
                            <option value="any">Any Health Status</option>
                            <option value="yes">Verified</option>
                            <option value="no">Unverified</option>
                        </select>
                        <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-gray-900/50 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-teal-500">
                        ...
                        <div className="flex items-center gap-2">
                             <input type="number" placeholder="Min Age" name="minAge" value={filters.minAge} onChange={handleFilterChange} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-teal-500" />
                             <input type="number" placeholder="Max Age" name="maxAge" value={filters.maxAge} onChange={handleFilterChange} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-teal-500" />
                        </div>
                         <div className="flex items-center gap-2">
                             <input type="number" placeholder="Min Weight" name="minWeight" value={filters.minWeight} onChange={handleFilterChange} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-teal-500" />
                             <input type="number" placeholder="Max Weight" name="maxWeight" value={filters.maxWeight} onChange={handleFilterChange} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-teal-500" />
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                 ...
                 {filteredAndSortedCattle.length === 0 && (
                    <div className="text-center col-span-full py-12">
                        <p className="text-xl text-gray-400">No cattle match the current filters.</p>
                    </div>
                )}
            </div>
        </>
